...
describe("Crisis handling", function() {
    ...
    it("Quorum not reached, fail to execute without commission", async function() {
        ...
        for (let i = 1; i < 3; ++i) {
            await governor.connect(addresses[i]).castVoteWithReason(proposalId, VoteType.Against, "voted for")
        }
        for (let i = 3; i < 6; ++i) {
            await governor.connect(addresses[i]).castVoteWithReason(proposalId, VoteType.Abstain, "voted for")
        }
        ...
        let hash =  await governor.decisionHash(proposalId, 1);
        ...
        // submit decision
        await expect(governor.submit([unit.address], [0], [encodedChangeState], descriptionHash, 1, signatures)).not.to.be.reverted

        assert.equal(await governor.state(proposalId), ProposalState.Succeeded, "Proposal state expected to be `Succeeded`")
        /* todo:
            - tests for successfulCommissionGathering
